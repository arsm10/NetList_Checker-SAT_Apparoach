...
void counter(vector<int> value)
{
	...
	for(size_t i = 0; i < inputs1.size(); i++)
	{
		cout << inputs1[i] << ":" << value[map2[inputs1[i]] + netCount1] << "\n";
	}
	cout << "\nOutputs: " << "\n" << "Netlist 2 \n";
...
void DP(vector< vector<int> > cnf, vector<int> value)
{
	...
	UnitClauseloop:
	unitc = 0;
	//for loop to check the clauses in CNF.
	...
	/*cout << "Cnf after removing Unit Clause \n";
	...
	//Calling the last element of the CNF by using '.back()' and it will store in an integer.
	assign_value = abs(cnf.back().back());
	//For back tracking introduce new CNF.
	vector< vector<int> > cnf1;
	//New CNF value is equal to the old CNF value after removing Unit Clause
	cnf1 = cnf;
	cout << "\nLast Literal: " << assign_value << "\n";
	value[assign_value] = 0;

	for(size_t clause = 0; clause < cnf.size(); clause++)
	{
		for(size_t lit = 0; lit < cnf[clause].size(); lit++)
		{
			if(cnf[clause][lit] == -assign_value)
			{
	...
	cout << "\nnow to 1: " << assign_value <<"\n";

	for(size_t clause = 0; clause < cnf1.size(); clause++)
	{
		for(size_t lit = 0; lit < cnf1[clause].size(); lit++)
		{
			if(cnf1[clause][lit] == assign_value)
			{
				cnf1.erase(cnf1.begin() + clause);
				clause--;
				break;
			}
			if(cnf1[clause][lit] == -assign_value)
			{
				cnf1[clause].erase(cnf1[clause].begin() + lit);
			}
		}
	}
	/*//for loop to check total number to clauses after erasing
...
int main(int argc, char ** argv)
{
	...
	for(size_t i = 0; i < gates2.size(); i++)
	{
		...
		case INV:
			p0.push_back(netCount1 + gates2[i].nets[0]);
			p0.push_back(netCount1 + gates2[i].nets[1]);
			cnf.push_back(p0);

			p1.push_back(-netCount1 - gates2[i].nets[0]);
			p1.push_back(-netCount1 - gates2[i].nets[1]);
			cnf.push_back(p1);
			break;
			//Characteristic function of XOR(a,b,c) = (-a OR b OR c) AND (a OR -b OR c) AND (a OR b OR -c) AND (-a OR -b OR -c)
		case XOR:
			...
			p3.push_back(-netCount1 - gates2[i].nets[0]);
			p3.push_back(-netCount1 - gates2[i].nets[1]);
			p3.push_back(-netCount1 - gates2[i].nets[2]);
			cnf.push_back(p3);
			break;
		}
	}
